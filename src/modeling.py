...
def scale_features(
    ...
    If target_columns is None, it defaults to scaling only FLOAT columns.
    It avoids scaling integer columns (often one-hot encoded or ordinal) unless specified.
    """
    if target_columns is None:
        # Select only float columns to avoid scaling dummy/integer encoded features
...
def train_simple_model(
    ...
    **kwargs
    ...
    """
    Train a model (logistic, random forest, gradient boosting, linear) 
    with optional SMOTE for imbalance.
    """
    X_train_res, y_train_res = X_train.copy(), y_train.copy()
    
    # Handle imbalance for classification using SMOTE
    if problem == "classification" and resample:
        ...
        try:
            ...
            logging.warning(f"SMOTE failed (likely too few minority samples), proceeding without: {e}")

    # Initialize model
    model = None
    if problem == "classification":
        if model_name == "logistic":
            # Set default class_weight if not provided in kwargs
    ...
    if hasattr(model, "feature_importances_"):
        ...
        # For linear models, coef_ might be 2D
        coefs = model.coef_[0] if model.coef_.ndim > 1 else model.coef_
        importances = pd.Series(coefs, index=X_train_res.columns)
        logging.info("Top 5 coefficients (abs):\n" + str(importances.abs().sort_values(ascending=False).head(5)))
