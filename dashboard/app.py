...
#         # Check if Age is used by model
#         cust_age = 0
#         # Ensure 'Customer_Age' is in the scaler's feature names (which it would be if it was used in training)
#         # and also in the model_features list, which represents the final features used by the model.
...
#             "Transaction_Amount": txn_amount,
#             "Account_Balance": acc_balance,
#             # Ensure Customer_Age is only included if it was expected by the scaler/model
#             "Customer_Age": cust_age if "Customer_Age" in scaler.feature_names_in_ else 0 
#         }
...
#             # Align columns for scaling specifically with what the scaler expects
#             final_raw_for_scaling = pd.DataFrame()
#             for c in scaler.feature_names_in_:
#                 # Use .get(c, 0) for robustness if a column is missing from raw_df_for_scaling
...
#         # 5. Predict
#         # Ensure X_final contains only the features the model was trained on
...
#         # ==========================================
#         # 7. LOG TO MLFLOW (FULL DATASET, MODEL, METRICS)
#         # ==========================================
#         try:
#             with mlflow.start_run():
#                 # --- A. LOG METRICS (Performance & Results) ---
#                 mlflow.log_metric("fraud_probability", prob)
#                 mlflow.log_metric("inference_latency_ms", latency_ms) # How fast was the prediction?
                
#                 # --- B. LOG PARAMS (High-level inputs for easy filtering) ---
#                 mlflow.log_params(raw_fin) # Log the raw financial inputs
#                 mlflow.log_param("Hour", hour)
#                 mlflow.log_param("Weekday", weekday)
#                 mlflow.log_params(user_categoricals) # Log the selected categorical values
#                 mlflow.log_param("prediction_class", "Fraud" if is_fraud else "Legit")
                
#                 # --- C. LOG DATASET (The complete input features as an artifact) ---
#                 # This captures the exact feature vector that was fed to the model,
#                 # along with the raw inputs for debugging and reproducibility.
#                 dataset_snapshot = {
#                     "raw_user_inputs": {
#                         "Transaction_Amount": txn_amount,
#                         "Account_Balance": acc_balance,
...
#                         **user_categoricals # Merge user selected categoricals
#                     },
#                     "model_input_vector_scaled_and_encoded": input_vector,
#                     "prediction_probability": float(prob),
#                     "is_fraud_prediction": bool(is_fraud)
#                 }
#                 # Save as a JSON artifact
#                 mlflow.log_dict(dataset_snapshot, "inference_data.json")

#                 # --- D. LOG MODEL (Optional for inference runs) ---
#                 # NOTE: Logging the model every time can be very slow and consume a lot of storage.
#                 # In a production setup, the model is usually logged once during the training run,
#                 # and then referenced by its run_id or name in the inference application.
#                 # I'm including this as per your request, but be mindful of its implications.
...
#                 mlflow.set_tag("model_name", "rf_model_imbalanced") # Assuming this is your model name
...
experiment_name = "Live_Dashboard_Inference"
